// lib/VoyageScreen.dart - AVEC VISITEUR + TÉLÉCHARGEMENT
// ⭐ CORRIGÉ : Partage DIRECT vers l'app choisie (sans menu intermédiaire)
...
class _VoyageScreenState extends State<VoyageScreen>
  ...
  void initState() {
    super.initState();
    debugPrint(
        '📌 VoyageScreen - isVisitor: ${widget.isVisitor}, canDownload: ${widget.canDownload}');
  ...
  // ============================================================
  // ⭐⭐⭐ PARTAGE DIRECT VERS L'APP CHOISIE ⭐⭐⭐
  // ============================================================
  void _shareOnSocial(Map<String, dynamic> app) async {
    ...
    final String appName = app['name']?.toString() ?? '';
    final String firstUrl = selectedUrls.first;

    final String messageText =
        '📸 Mes photos - ${widget.categoryName}\n\n${selectedUrls.join('\n\n')}';

    final String encodedMsg = Uri.encodeComponent(messageText);
    final String encodedUrl = Uri.encodeComponent(firstUrl);
    ...
    switch (appName) {
      case 'WhatsApp':
        directUrl = 'whatsapp://send?text=$encodedMsg';
        fallbackUrl = 'https://wa.me/?text=$encodedMsg';
        break;

      case 'Telegram':
        directUrl = 'tg://msg?text=$encodedMsg';
        fallbackUrl = 'https://t.me/share/url?url=$encodedUrl&text=$encodedMsg';
        break;
      ...
      case 'Messenger':
        directUrl = 'fb-messenger://share?link=$encodedUrl';
        fallbackUrl = 'https://www.messenger.com/';
        break;

      case 'Instagram':
        directUrl = 'instagram://app';
        fallbackUrl = 'https://www.instagram.com/';
        break;

      default:
        if (mounted) setState(() => _selectedPhotos = {});
        return;
    }
    ...
    try {
      ...
      if (await canLaunchUrl(uri)) {
        await launchUrl(uri, mode: LaunchMode.externalApplication);
        debugPrint('✅ $appName ouvert directement');
      } else if (fallbackUrl != null) {
        final fallbackUri = Uri.parse(fallbackUrl);
        if (await canLaunchUrl(fallbackUri)) {
          await launchUrl(fallbackUri, mode: LaunchMode.externalApplication);
          debugPrint('✅ $appName ouvert via web');
        } else {
          await Share.share(messageText);
        }
      }
    } catch (e) {
      debugPrint('❌ Erreur partage $appName: $e');
      try {
        await Share.share(messageText);
      } catch (_) {}
    }
