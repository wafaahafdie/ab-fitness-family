// lib/AjouterDocumentVScreen.dart
// ⭐ Version VIDÉO - Catégories familiales
...
// ============================================================
// MODELE CATEGORIE VIDEO
// ============================================================

class CategorieVideoIcone {
  final IconData icone;
  final String nom;
  final Color couleur;
  const CategorieVideoIcone(this.icone, this.nom, this.couleur);
}

// ============================================================
// AJOUTER DOCUMENT V SCREEN (pour MES VIDÉOS)
// ============================================================

class AjouterDocumentVScreen extends StatefulWidget {
  final String? memberId;
  final String? memberDocId;
  final String? memberCollection;

  const AjouterDocumentVScreen({
    super.key,
    this.memberId,
    this.memberDocId,
  ...
  @override
  State<AjouterDocumentVScreen> createState() => _AjouterDocumentVScreenState();
}

class _AjouterDocumentVScreenState extends State<AjouterDocumentVScreen> {
  // ==========================================================
  // CONTROLLER
  // ==========================================================
  ...
  // ==========================================================
  // ICONES FAMILIALES POUR VIDÉOS
  // ==========================================================

  final List<CategorieVideoIcone> _icones = const [
    // 🎉 Fêtes & Événements
    CategorieVideoIcone(Icons.cake_outlined, 'Anniversaire', Color(0xFFFF7D27)),
    CategorieVideoIcone(Icons.celebration_outlined, 'Fête', Color(0xFFE91E9B)),
    CategorieVideoIcone(Icons.favorite_border, 'Mariage', Color(0xFFE91E9B)),
    CategorieVideoIcone(Icons.church_outlined, 'Baptême', Color(0xFF4777F5)),
    CategorieVideoIcone(
    ...
    // ✈️ Voyages & Vacances
    CategorieVideoIcone(Icons.flight, 'Voyage', Color(0xFF10B8AD)),
    CategorieVideoIcone(
    ...
    CategorieVideoIcone(Icons.hotel_outlined, 'Hôtel', Color(0xFF9B59B6)),
    CategorieVideoIcone(
        Icons.camera_alt_outlined, 'Souvenirs', Color(0xFFE91E63)),

    // 👨‍👩‍👧‍👦 Famille & Amis
    CategorieVideoIcone(Icons.groups_outlined, 'Famille', Color(0xFF8048E9)),
    CategorieVideoIcone(Icons.people_outline, 'Amis', Color(0xFF4777F5)),
    CategorieVideoIcone(
    ...
    CategorieVideoIcone(Icons.pool_outlined, 'Natation', Color(0xFF00BCD4)),
    CategorieVideoIcone(Icons.hiking_outlined, 'Randonnée', Color(0xFF27AE60)),

    // 🏠 Vie quotidienne
    CategorieVideoIcone(Icons.home_outlined, 'Maison', Color(0xFF8E44AD)),
    CategorieVideoIcone(Icons.pets, 'Animaux', Color(0xFFE67E22)),
    CategorieVideoIcone(Icons.work_outline, 'Travail', Color(0xFF1F2A6B)),
    CategorieVideoIcone(
        Icons.local_florist_outlined, 'Jardin', Color(0xFF62C93D)),

    // 📸 Autres / Divers
    CategorieVideoIcone(
        Icons.video_library_outlined, 'Vidéo', Color(0xFFE74C3C)),
    CategorieVideoIcone(Icons.movie_outlined, 'Film', Color(0xFF9B59B6)),
    CategorieVideoIcone(Icons.star_outline, 'Favoris', Color(0xFFF1C40F)),
    CategorieVideoIcone(Icons.videocam_outlined, 'Vrac', Color(0xFF2C3E50)),
  ];
  ...
  // ==========================================================
  // ⭐ CRÉER LA CATÉGORIE VIDÉO + SAUVEGARDER DANS FIRESTORE
  // ==========================================================
    ...
    try {
      ...
      if (widget.memberDocId != null &&
        ...
        final alreadyExists = existing.any(
          (c) => c is Map && c['nom'] == nom && c['source'] == 'videos_screen',
        );

        if (!alreadyExists) {
          existing.add(nouvelleCategorie);
          await memberRef.update({'categories': existing});
          debugPrint('✅ Catégorie "$nom" enregistrée dans le membre');
        } else {
          debugPrint('⚠️ Catégorie "$nom" existe déjà');
        }
      } else {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ Catégorie "$nom" créée'),
