// lib/Diplome.dart - SÉLECTION MULTIPLE + TOUT SÉLECTIONNER + PARTAGE SOCIAL
...
class DiplomeFile {
  ...
  factory DiplomeFile.fromJson(Map<String, dynamic> json) {
    return DiplomeFile(
      name: json['name'] ?? '',
      size: json['size'] ?? '',
      date: json['date'] ?? '',
  ...
  factory DiplomeFile.fromFirestore(String id, Map<String, dynamic> data) {
    DateTime? parsedDate;
    try {
      parsedDate =
...
class DiplomeScreen extends StatefulWidget {
  ...
  final String memberId;
  final List<DiplomeFile>? initialFiles;
  ...
  @override
  State<DiplomeScreen> createState() => _DiplomeScreenState();
}

class _DiplomeScreenState extends State<DiplomeScreen> {
  late List<DiplomeFile> files;
  ...
  static const String _firestoreCollection = 'DiplomeFiles';
  static const String _supabaseBucket = 'diplome_files';
  ...
  int _compareByDateDesc(DiplomeFile a, DiplomeFile b) {
  ...
  Future<void> _loadFromFirestore() async {
    try {
      ...
      final List<DiplomeFile> loadedFiles = querySnapshot.docs.map((doc) {
        return DiplomeFile.fromFirestore(doc.id, doc.data());
      }).toList();
  ...
  void _listenToFirestoreRealtime() {
    ...
    _firestoreSubscription = FirebaseFirestore.instance
      ...
      debugPrint('❌ Erreur écoute Firestore: $error');
      if (mounted) setState(() => _isLoading = false);
    });
  }
  ...
  // ═══════════════════════════════════════════════════════
  // ⭐⭐⭐ NOUVEAU : TÉLÉCHARGER UN SEUL DIPLÔME ⭐⭐⭐
  // Appelé par l'icône ⬇️ sur chaque diplôme (visiteur)
  // ═══════════════════════════════════════════════════════

  Future<void> _telechargerFichier(DiplomeFile file) async {
  ...
  Future<void> _importerDiplome() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Diplôme importé avec succès'),
            backgroundColor: Colors.green,
          ),
        );
      }
  ...
  Future<void> _confirmSupprimer(DiplomeFile file) async {
    if (widget.isVisitor) return;
  ...
  Future<void> _ouvrirFichier(DiplomeFile file) async {
    try {
      if (file.downloadUrl != null && file.downloadUrl!.isNotEmpty) {
        final uri = Uri.parse(file.downloadUrl!);
        await launchUrl(uri, mode: LaunchMode.externalApplication);
  ...
  Future<void> _renommerFichier(DiplomeFile file) async {
    if (widget.isVisitor) return;
    if (kIsWeb) return;
  ...
  void _partagerFichier(DiplomeFile file) {
    if (widget.isVisitor) return;
  ...
  void _showShareSheet(List<DiplomeFile> fichiers) {
  ...
  Future<void> _copyLinks(List<DiplomeFile> fichiers) async {
  ...
  Future<void> _shareOnSocial(
      Map<String, dynamic> app, List<DiplomeFile> fichiers) async {
    ...
    final String message = validFiles.length == 1
        ...
        : '📄 Mes diplômes (${validFiles.length})\n\n${validFiles.map((f) => '• ${f.name}\n${f.downloadUrl}').join('\n\n')}';
    ...
    return Row(
      children: [
        Container(
          ...
          child: Center(
              child: _asset('dip',
                  size: 26, color: purple, fallback: Icons.school_outlined)),
        ),
  ...
  // ⭐⭐⭐ FILE TILE : icône ⬇️ par diplôme (visiteur) ⭐⭐⭐
  Widget _fileTile(DiplomeFile file) {
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            ...
            // ⭐⭐⭐ ICÔNE ⬇️ PAR DIPLÔME (VISITEUR SEULEMENT) ⭐⭐⭐
  ...
  Widget _buildFileMenu(DiplomeFile file) {
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: Column(
        children: [
          _asset('dip',
              size: 40, color: purple, fallback: Icons.school_outlined),
          const SizedBox(height: 10),
          const Text('Aucun diplôme pour le moment',
              style: TextStyle(
                  color: navy, fontWeight: FontWeight.w700, fontSize: 13)),
          const SizedBox(height: 4),
          Text(
            widget.isVisitor
                ? 'Aucun diplôme à consulter'
  ...
  Widget _buildBottomButtons() {
    ...
    return GestureDetector(
      onTap: _isImporting ? null : _importerDiplome,
      child: Container(
        width: double.infinity,
        height: 46,
        decoration: BoxDecoration(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text(_isImporting ? 'Import en cours...' : 'Importer un diplôme',
                style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.w700,
                    fontSize: 13)),
