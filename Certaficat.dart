// lib/Certaficat.dart - SÉLECTION MULTIPLE + TOUT SÉLECTIONNER + PARTAGE SOCIAL
// ⭐ CORRIGÉ : Import PDF depuis téléphone (lecture depuis path si bytes null)
import 'dart:async';
import 'dart:convert';
import 'dart:io'; // ⭐ AJOUTÉ pour File
...
// ============================================================
// MODELE FICHIER CERTIFICAT
// ============================================================

class CertificatFile {
  final String name;
  final String size;
  final String date;
  final String path;
  ...
  String get dateLabel {
    if (modifiedDate == null) return date;
    const mois = [
      'Jan',
      'Fév',
      'Mar',
    ...
    return '${modifiedDate!.day.toString().padLeft(2, '0')} ${mois[modifiedDate!.month - 1]} ${modifiedDate!.year}';
  }
  ...
  factory CertificatFile.fromJson(Map<String, dynamic> json) {
  ...
  Map<String, dynamic> toFirestore() {
    return {
      ...
      'sizeBytes': sizeBytes,
      'date':
          modifiedDate?.toIso8601String() ?? DateTime.now().toIso8601String(),
      'memberId': memberId,
      'path': path,
  ...
  factory CertificatFile.fromFirestore(String id, Map<String, dynamic> data) {
    DateTime? parsedDate;
    try {
      parsedDate =
          data['date'] != null ? DateTime.parse(data['date'].toString()) : null;
    } catch (_) {
      parsedDate = null;
    }
...
class Certaficat extends StatefulWidget {
  final String ownerName;
  final String memberId;
  final List<CertificatFile>? initialFiles;
  ...
  @override
  State<Certaficat> createState() => _CertaficatState();
}

class _CertaficatState extends State<Certaficat> {
  late List<CertificatFile> files;
  ...
  static const String _firestoreCollection = 'CertificatFiles';
  static const String _supabaseBucket = 'certificat_files';
  ...
  int _compareByDateDesc(CertificatFile a, CertificatFile b) {
    final epoch = DateTime.fromMillisecondsSinceEpoch(0);
    final aDate = a.modifiedDate ?? epoch;
    final bDate = b.modifiedDate ?? epoch;
  ...
  Future<void> _loadFromFirestore() async {
    try {
      ...
      final List<CertificatFile> loadedFiles = querySnapshot.docs.map((doc) {
        return CertificatFile.fromFirestore(doc.id, doc.data());
      }).toList();

      loadedFiles.sort(_compareByDateDesc);
  ...
  // ═══════════════════════════════════════════════════════
  // ⭐ IMPORT CERTIFICAT - CORRIGÉ POUR ANDROID
  // ═══════════════════════════════════════════════════════

  Future<void> _importerCertificat() async {
    ...
    try {
      ...
      // ⭐⭐⭐ CORRECTION : essayer bytes, puis lire depuis path
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Certificat importé avec succès'),
  ...
  Future<void> _confirmSupprimer(CertificatFile file) async {
    if (widget.isVisitor) return;
  ...
  Future<void> _ouvrirFichier(CertificatFile file) async {
    try {
      if (file.downloadUrl != null && file.downloadUrl!.isNotEmpty) {
        final uri = Uri.parse(file.downloadUrl!);
        final opened =
            await launchUrl(uri, mode: LaunchMode.externalApplication);
        if (!opened) throw Exception('Impossible d\'ouvrir le PDF');
      }
    } catch (e) {
  ...
  Future<void> _renommerFichier(CertificatFile file) async {
    if (widget.isVisitor) return;
    if (kIsWeb) return;

    final ext = p.extension(file.name);
    final controller =
        TextEditingController(text: p.basenameWithoutExtension(file.name));
  ...
  void _partagerFichier(CertificatFile file) {
    if (widget.isVisitor) return;
  ...
  void _showShareSheet(List<CertificatFile> fichiers) {
  ...
  Future<void> _copyLinks(List<CertificatFile> fichiers) async {
  ...
  Future<void> _shareOnSocial(
      Map<String, dynamic> app, List<CertificatFile> fichiers) async {
    ...
    final String message = validFiles.length == 1
        ...
        : '📄 Mes certificats (${validFiles.length})\n\n${validFiles.map((f) => '• ${f.name}\n${f.downloadUrl}').join('\n\n')}';
  ...
  Widget _buildTopBar(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Row(
          children: [
            if (widget.isVisitor && widget.canDownload && files.isNotEmpty)
              GestureDetector(
                onTap: _isSelecting ? _exitSelectionMode : _enterSelectionMode,
                child: Container(
                  padding:
                      const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                  decoration: BoxDecoration(
                    color: _isSelecting ? Colors.red : purple,
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        _isSelecting ? 'Annuler' : 'Télécharger',
                        style: const TextStyle(
                            color: Colors.white,
                            fontSize: 12,
  ...
  Widget _buildFilesList() {
    return Column(children: files.map((f) => _fileTile(f)).toList());
  }

  Widget _fileTile(CertificatFile file) {
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: const Center(
                  child:
                      Icon(Icons.verified_outlined, color: purple, size: 22)),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
  ...
  Widget _buildFileMenu(CertificatFile file) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const Text('Aucun certificat pour le moment',
              style: TextStyle(
                  color: navy, fontWeight: FontWeight.w700, fontSize: 13)),
          const SizedBox(height: 4),
          Text(
            widget.isVisitor
                ? 'Aucun certificat à consulter'
  ...
  Widget _buildBottomButtons() {
    ...
    return GestureDetector(
      onTap: _isImporting ? null : _importerCertificat,
      child: Container(
        width: double.infinity,
        height: 46,
        decoration: BoxDecoration(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text(_isImporting ? 'Import en cours...' : 'Importer un certificat',
                style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.w700,
                    fontSize: 13)),
