// lib/FamilleScreen.dart - AVEC VISITEUR + TÉLÉCHARGEMENT
...
// ============================================================
// FAMILLE SCREEN - AVEC VISITEUR + TÉLÉCHARGEMENT
// ============================================================
...
class _FamilleScreenState extends State<FamilleScreen>
  ...
  void initState() {
    super.initState();
    debugPrint(
        '📌 FamilleScreen - isVisitor: ${widget.isVisitor}, canDownload: ${widget.canDownload}');
  ...
  // ============================================================
  // ⭐ PARTAGE SOCIAL CORRIGÉ — OUVERTURE DIRECTE DES APPS
  // WhatsApp / Telegram : texte pré-rempli ✅
  // Instagram : ouverture directe (pas de texte) ✅
  // Facebook / Messenger : lien uniquement
  // ============================================================
  void _shareOnSocial(Map<String, dynamic> app) async {
    ...
    // Message complet avec tous les liens
    final String messageText =
        '📸 Mes photos - ${widget.categoryName}\n\n${selectedUrls.join('\n\n')}';

    final String encodedText = Uri.encodeComponent(messageText);
    final String encodedFirstUrl = Uri.encodeComponent(selectedUrls.first);
    ...
    try {
      switch (app['name']) {
        // ============================================================
        // WHATSAPP — texte pré-rempli ✅
        // ============================================================
        case 'WhatsApp':
          final waApp = Uri.parse('whatsapp://send?text=$encodedText');
          if (await canLaunchUrl(waApp)) {
            await launchUrl(waApp, mode: LaunchMode.externalApplication);
            launched = true;
          } else {
            final waWeb = Uri.parse('https://wa.me/?text=$encodedText');
        ...
        // ============================================================
        // TELEGRAM — texte pré-rempli ✅
        // ============================================================
        case 'Telegram':
          final tgApp = Uri.parse('tg://msg?text=$encodedText');
          if (await canLaunchUrl(tgApp)) {
            await launchUrl(tgApp, mode: LaunchMode.externalApplication);
            launched = true;
          } else {
            final tgWeb = Uri.parse(
                'https://t.me/share/url?url=$encodedFirstUrl&text=$encodedText');
        ...
        // ============================================================
        // FACEBOOK — lien uniquement (pas de texte libre)
        // ============================================================
        case 'Facebook':
          final fbApp = Uri.parse('fb://share?link=$encodedFirstUrl');
          if (await canLaunchUrl(fbApp)) {
            await launchUrl(fbApp, mode: LaunchMode.externalApplication);
            launched = true;
          } else {
            final fbWeb = Uri.parse(
                'https://www.facebook.com/sharer/sharer.php?u=$encodedFirstUrl');
            if (await canLaunchUrl(fbWeb)) {
        ...
        // ============================================================
        // MESSENGER — lien uniquement
        // ============================================================
        case 'Messenger':
          final messengerApp =
              Uri.parse('fb-messenger://share?link=$encodedFirstUrl');
          if (await canLaunchUrl(messengerApp)) {
            await launchUrl(messengerApp, mode: LaunchMode.externalApplication);
            launched = true;
          } else {
            final messengerWeb = Uri.parse(
                'https://www.facebook.com/dialog/send?link=$encodedFirstUrl'
                '&app_id=291494419107518'
                '&redirect_uri=https://www.facebook.com/');
            if (await canLaunchUrl(messengerWeb)) {
        ...
        // ============================================================
        // INSTAGRAM — ouverture directe, PAS de texte pré-rempli
        // (limitation officielle d'Instagram)
        // ============================================================
        case 'Instagram':
          final igApp = Uri.parse('instagram://app');
          if (await canLaunchUrl(igApp)) {
            await launchUrl(igApp, mode: LaunchMode.externalApplication);
            launched = true;
          } else {
            final igWeb = Uri.parse('https://www.instagram.com/');
            if (await canLaunchUrl(igWeb)) {
      ...
      debugPrint('❌ Erreur partage ${app['name']}: $e');
    }

    // 🔄 Fallback : app non installée → partage natif
    if (!launched) {
      try {
        await Share.share(messageText);
      } catch (e) {
        debugPrint('❌ Erreur Share.share: $e');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  '❌ Impossible d\'ouvrir ${app['name']} sur cet appareil'),
            ),
          );
        }
      }
      ...
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(widget.categoryName,
              style: const TextStyle(
                  color: navy, fontSize: 18, fontWeight: FontWeight.w700)),
          backgroundColor: Colors.white,
          elevation: 0,
          centerTitle: true,
        ),
